...
import threading              # 스레드 기능을 위해 import
import time                   # sleep()을 쓰기 위해서
import random


class Unit:
    def __init__(self, pp, mp, ph, mh, hp):
        self.pPower = pp       # 물리 공격력
        self.mPower = mp       # 마법 공격력
        self.pHit = ph         # 물리 적중률
        self.mHit = mh         # 마법 적중률
        self.hp = hp           # 체력
...
if __name__ == "__main__":  # main thread 흐름을 타는 시작점
    ...
    print("높은 체력과 강력한 물리 공격력을 가진 캐릭터 전사")
    name1 = input("전사의 이름 입력 : ")
    print("높은 체력과 강력한 물리 공격력을 가진 캐릭터")
    name2 = input("마법사의 이름 입력 : ")

    warrior = Character(8, 2, 0.8, 0.5, 150, 40, name1)  # 물공,마공, 물방, 마방, 물적,마적,체력,궁극기
    wizard = Character(2, 20, 0.5, 0.9, 70, 55, name2)  # 물공,마공, 물방, 마방, 물적,마적,체력,궁극기
